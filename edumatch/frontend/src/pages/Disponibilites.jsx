...
@keyframes ttIn     { from{opacity:0;transform:scale(.92)} to{opacity:1;transform:scale(1)} }
...
.dp-cal-day.clickable:hover { transform:scale(1.14); z-index:3; }
...
.dp-tt {
  ...
  background:#0F172A; border:1.5px solid #1E293B; border-radius:16px;
  width:260px;
  box-shadow:0 24px 60px rgba(0,0,0,.5);
  animation:ttIn .16s cubic-bezier(.22,1,.36,1) both;
  overflow:hidden;
}
...
function isPast(dateStr, heureFinStr) {
  ...
  const seanceEnd = new Date(y, m-1, d, hh, mm);
  return seanceEnd < now;
}
...
// ─── Positionner le tooltip de façon intelligente ───────────
// Règle : afficher au-dessus si assez de place, sinon en-dessous
// Toujours dans les limites de l'écran horizontalement
function calcTooltipPos(rect, ttW=260, ttH) {
  const margin = 12;
  ...
  if(spaceAbove >= ttH) {
    top = rect.top - ttH - margin;       // au-dessus
  } else if(spaceBelow >= ttH) {
    top = rect.bottom + margin;           // en-dessous
  } else {
    // Centre verticalement dans la fenêtre
    top = Math.max(margin, (vp.h - ttH) / 2);
  }

  // Position horizontale : centrer sur l'élément, clamp dans la fenêtre
  let left = cx - ttW / 2;
  left = Math.max(margin, Math.min(left, vp.w - ttW - margin));
...
function CalTooltip({ seances, niveaux, rect }) {
  ...
  useEffect(() => {
    ...
    setPos(calcTooltipPos(rect, TT_W, h));
  }, [rect, seances]);

  if(!seances?.length || !rect) return null;

  // Tri : à venir en premier, passés en dernier
  ...
  return (
    <div ref={ref} className="dp-tt"
      ...
      <div style={{ padding:'10px 14px', borderBottom:'1px solid #1E293B', display:'flex', justifyContent:'space-between', alignItems:'center' }}>
        ...
        <span style={{ fontSize:'.7rem', color:'#475569', fontWeight:600 }}>
        ...
        {sorted.map((d,i) => {
          ...
          return (
            <div key={i} style={{
              ...
              background: past ? '#1E293B' : '#1E293B',
              border: `1.5px solid ${past ? '#334155' : o.col+'44'}`,
              opacity: past ? .6 : 1,
            }}>
              {/* Heure + statut */}
              <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:4 }}>
                ...
                {past ? (
                  <span style={{ fontSize:'.62rem', fontWeight:800, color:'#475569', background:'#334155', padding:'1px 7px', borderRadius:20 }}>Passé</span>
                ) : (
                  <span style={{ fontSize:'.62rem', fontWeight:800, padding:'1px 7px', borderRadius:20, background:`${o.col}22`, color:o.col }}>
                    {o.label}
                  </span>
                )}
              </div>
              {/* Niveau */}
              {niv && <div style={{ fontSize:'.68rem', color:'#64748B', marginBottom:5, fontWeight:600 }}>🎓 {niv.nom}</div>}
              {/* Barre capacité */}
              {!past && d.nb_max_etudiants>0 && (
                <div style={{ display:'flex', alignItems:'center', gap:7 }}>
                  ...
                  <span style={{ fontSize:'.66rem', fontWeight:800, color:o.col, whiteSpace:'nowrap' }}>
...
// ─── Calendrier ─────────────────────────────────────────────
function SmartCalendar({ dispos, niveaux }) {
  ...
  // Tooltip state : { seances, rect } ou null
  ...
  // Map jour → liste dispos (toutes, passées incluses pour l'historique)
  const dayMap = {};
  dispos.forEach(d=>{
  ...
  // Statut d'un jour (basé sur les séances NON passées uniquement)
  function dayStatus(list) {
    if(!list?.length) return null;
    const future = list.filter(d=>!isPast(d.date_specifique, d.heure_fin));
    if(!future.length) return 'past'; // toutes passées
  ...
  const handleEnter = useCallback((e, list) => {
    ...
    setTT({ seances:list, rect });
  },[]);

  const handleLeave = useCallback(()=>setTT(null),[]);
    ...
    <div style={{ position:'relative' }}>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(7,1fr)', gap:3 }}>
        {cells.map((d,i)=>{
          ...
          const dayPast = isDatePast(dateStr) && !tod; // jour passé (pas aujourd'hui)
          ...
          return (
            <div key={i}
              ...
              style={{
                ...
                // Les jours passés sans séances sont très discrets
      ...
      {/* Tooltip rendu en portal-like (position:fixed dans CalTooltip) */}
      {tt && <CalTooltip seances={tt.seances} niveaux={niveaux} rect={tt.rect}/>}
    </div>
  );
}
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1.45fr .55fr', gap:22 }}>
              ...
              .map(([dateKey,seances])=>(
                <div key={dateKey} style={{ marginBottom:20 }}>
                  ...
                  {seances.map((d,idx)=>{
                    ...
                    return (
                      <div key={d.id} className={`dp-card${past?' past':''}`}
                        style={{ animationDelay:`${idx*40}ms`, animation:'slideIn .35s cubic-bezier(.22,1,.36,1) both' }}>
                        {/* Barre latérale */}
                        <div style={{ position:'absolute', left:0, top:0, bottom:0, width:4, borderRadius:'4px 0 0 4px', background:past?'#CBD5E1':o.col }}/>

                        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', paddingLeft:8 }}>
                          <div style={{ flex:1 }}>
                            {/* Heure + durée + badge statut */}
                            <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:8, flexWrap:'wrap' }}>
                              <span style={{ fontFamily:'Cabinet Grotesk,sans-serif', fontWeight:900, color:past?'#64748B':'#0F172A', fontSize:'1rem' }}>
                                🕒 {d.heure_debut?.slice(0,5)} – {d.heure_fin?.slice(0,5)}
                              </span>
                              {durL && (
                              ...
                              {past ? (
                                <span style={{ marginLeft:'auto', fontSize:'.65rem', fontWeight:800, padding:'2px 9px', borderRadius:20, background:'#F1F5F9', color:'#94A3B8', border:'1.5px solid #E2E8F0' }}>
                                  ✓ Terminée
                                </span>
                              ) : (
                            ...
                            {/* Mode + niveau */}
                            <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:past||!d.description?6:8, flexWrap:'wrap' }}>
                            ...
                            {/* Description (masquée si passé pour alléger) */}
                            {d.description && !past && (
                            ...
                            {/* Résumé séance passée */}
                            {past && (
                              <div style={{ fontSize:'.72rem', color:'#94A3B8', fontWeight:600 }}>
                                {d.nb_inscrits>0
                                  ? `✓ ${d.nb_inscrits} étudiant${d.nb_inscrits>1?'s':''} avaient participé`
                                  : 'Aucun inscrit'}
                              </div>
                            )}
                          </div>
                          ...
                          <div style={{ display:'flex', flexDirection:'column', gap:6, marginLeft:12, flexShrink:0 }}>
                            {!past && <button className="dp-edit-btn" onClick={()=>setEditD(d)} title="Modifier">✏️</button>}
        ...
        <div style={{ position:'sticky', top:80, alignSelf:'start', display:'flex', flexDirection:'column', gap:16, animation:'fadeUp .5s .2s ease both' }}>
          ...
          <div style={{ background:'#fff', border:'1.5px solid #EEF2F8', borderRadius:24, padding:22, boxShadow:'0 2px 16px rgba(0,0,0,.04)' }}>
            ...
            <SmartCalendar dispos={dispos} niveaux={niveaux}/>
          </div>

          {/* Mini widget occupation */}
